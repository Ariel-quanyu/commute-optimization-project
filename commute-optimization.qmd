...
traffic_long <- traffic_long %>%
  mutate(
    date = as.Date(QT_INTERVAL_COUNT),
    interval_num = as.integer(substr(interval, 2, 3)),   
    hour = floor(interval_num / 4),                   
    weekday = wday(date, label = TRUE, abbr = FALSE)
  )
...
ggplot(traffic_by_hour, aes(x = hour, y = weekday, fill = avg_volume)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "green", high = "red") +
  labs(
    title = "Traffic Volume Heatmap by Hour and Weekday",
    subtitle = "Red = heavier traffic volume",
    x = "Hour of Day",
    y = "Day of Week",
    fill = "Vehicles/hr"
  ) +
...
ggsave("traffic_heatmap.png", plot = heatmap_plot, width = 8, height = 5, dpi = 300)
```
